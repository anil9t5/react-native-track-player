...
function add(tracks, trackID, insertBeforeId) {
    ...
    for(let i = 0; i < tracks.length; i++) {
        ...
        //TO DO: Initally play the first track i.e tracks.[i].url[0]..if user selects from the chapters, then pass the url[trackID]
        tracks[i].url = resolveUrl(tracks[i].url[0] ? tracks[i].url[0] : tracks[i].url[trackID]);
        tracks[i].artwork = resolveUrl(tracks[i].artwork);
